...
BATCH_SIZE = 5000
ROC_PLOT_PATH = os.path.join(OUTPUT_DIR, "roc_comparison_fulldf.png")
...
for name, path in MODEL_VARIANTS.items():
    ...
    X = df_full.drop(columns=['type']).copy()
    y_true = df_full['type'].map({'Phishing': 1, 'Legitimate': 0})
    ...
    preds['type'] = df_full['type']
    output_path = os.path.join(OUTPUT_DIR, f"predictions_{name}_full.csv")
...
metrics_csv_path = os.path.join(OUTPUT_DIR, "inference_metrics_summary_full.csv")
